...
BEGIN
		...
		INNER JOIN FormaPago fp WITH(NOLOCK) ON fp.id = CASE WHEN t.idFormaPago IS NULL THEN 2  ELSE t.idFormaPago END
		WHERE (@id IS NULL OR @id = c.id) ORDER BY t.fechaCreacion;
END